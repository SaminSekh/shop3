class AuditManager {
    ...
    setupEventListeners() {
        ...
        // Bulk Delete
        document.getElementById('selectAllAudit')?.addEventListener('change', (e) => this.toggleSelectAll(e.target.checked));
        document.getElementById('bulkDeleteAudit')?.addEventListener('click', () => this.handleBulkDelete());

        // Event delegation for checkboxes
        document.getElementById('auditTable')?.addEventListener('change', (e) => {
            if (e.target.classList.contains('audit-checkbox')) {
    ...
    renderAuditLogs(logs = this.auditLogs) {
        const tableBody = document.getElementById('auditTable');
        if (!tableBody) return;

        if (!logs || logs.length === 0) {
            tableBody.innerHTML = '<tr><td colspan="5" class="text-center">No login logs found</td></tr>';
            return;
        }
        ...
        tableBody.innerHTML = logs.map(log => {
            ...
            return `
                <tr>
                    <td><input type="checkbox" class="audit-checkbox" value="${log.id}"></td>
                    <td>
                        <div class="date-info">
    ...
    updateBulkDeleteButton() {
        const checkedCount = document.querySelectorAll('.audit-checkbox:checked').length;
        const deleteBtn = document.getElementById('bulkDeleteAudit');
        if (deleteBtn) {
    ...
    async handleBulkDelete() {
        ...
        if (!confirm(`Are you sure you want to delete ${ids.length} selected logs? This action cannot be undone.`)) {
            return;
        }
...
document.addEventListener('DOMContentLoaded', () => {
    // auditManager is already initialized above
});
