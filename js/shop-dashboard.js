...
class ShopDashboardManager {
    ...
    async loadOutstandingCredit() {
        try {
            // Total Credit: filtered by date range (shows credits created in this period)
            ...
            const { data: filteredCredits, error } = await creditQuery;
            if (error) throw error;

            const totalCreditFiltered = filteredCredits?.reduce((sum, c) => sum + parseFloat(c.total_amount || 0), 0) || 0;

            // Pending Balance: ALWAYS show ALL outstanding credits (not filtered by date)
            ...
            const totalPending = allPendingCredits?.reduce((sum, c) => sum + parseFloat(c.pending_amount || 0), 0) || 0;
    ...
    async loadSalesChart() {
        try {
            ...
            allSales.forEach(sale => {
                ...
                salesByDay[dayKey] = (salesByDay[dayKey] || 0) + (parseFloat(sale.total_amount) || 0);
            });
